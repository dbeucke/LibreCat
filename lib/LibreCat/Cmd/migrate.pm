...
sub command {
    ...
    if (
           !$store_config->{options}{bags}{session}
        || !$store_config->{options}{bags}{session}{plugins}
        || !array_includes(
            $store_config->{options}{bags}{session}{plugins},
            'Datestamps'
        )
  ...
  session:
    ...
    my $info_bag = Catmandu->store->bag('info');
    ...
    my $session_bag = Catmandu->store->bag('session');
